...
```
当 vyos 与 AWS 建立 IPSec VPN时，该私有 IP 会被 NAT 到一个公有 IP，在 vyos 上执行以下命令，将获取到公网 IP
```
aws@vyos-internet$ DX_PUBIP_VYOS=`curl http://checkip.amazonaws.com/` && echo ${DX_PUBIP_VYOS}
```

3. 通过 CloudShell 创建 IPSec VPN 的 Customer Gateway，注意请将 DX_PUBIP_VYOS 的参数 x.x.x.x 替换为上一步获取到的公网 IP
```
